...
<body>
	<div class="main">
			...
			<div id="main_contents">
				...
				<div id="con">
					<div id="contentsList">
						...
						<div
							<%=(type != null && type.equals("noContents")) ? ""
		: (postList.get(i).getP_lock().equals("y") ? "style='color:#707070'"
				: ("onclick=\"location.href=\'" + commons.baseUrl + "post?&name="
						+ URLEncoder.encode(boardName, "utf-8") + "&postId=" + postList.get(i).getP_id()
						+ "&view=new\'\""))%>
							class="<%=type != null && type.equals("noContents") ? "postNoContents" : "post"%>"
							title='<%=(type != null && type.equals("noContents")) ? ""
		: ("\"" + postList.get(i).getP_title() + (postList.get(i).getP_lock().equals("y") ? "*잠긴글*" : "") + "\"로 이동")%>'>
		...
		+ (postList.get(i).getP_lock().equals("y") ? "*잠긴글*" : "")%>
							...
							<p class="postWriter"
								<%=type != null && type.equals("noContents") ? "style='display:inline-block'" : ""%>><%=(postList.get(i).getU_nick().length() > 5 ? postList.get(i).getU_nick().substring(0, 6) + "..."
		: postList.get(i).getU_nick()) + "(" + postList.get(i).getU_id().substring(0, 3) + "***)"%></p>
							<div class="postInfo"
								<%=type != null && type.equals("noContents") ? "style='display:inline-block; float:right'" : ""%>>
								<%
					...
					<div id="write">
						...
						%>
						<a
							href="<%=commons.baseUrl%>write?&name=<%=URLEncoder.encode(boardName, "utf-8")%>">글쓰기</a>
						<%
							}
						%>
